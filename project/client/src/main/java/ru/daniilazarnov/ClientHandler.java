...
public class ClientHandler {
    ...
    public void chooseCommand(String msg) throws IOException {
        ...
        if (msg.startsWith(CommandList.UPLOAD.name().toLowerCase())) {
            ...
            RequestMsg rm = new RequestMsg(CommandList.DOWNLOAD, msgParts[1]);
            ...
            RequestMsg rm = new RequestMsg(CommandList.REMOVE, msgParts[1]);
            objectOut.writeObject(rm);
            objectOut.flush();
    ...
    private static void helpInfo() {
        ...
        LOGGER.info("remove - Удалить файлы на сервере (remove test1.txt)");
